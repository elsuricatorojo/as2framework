...
* Creada: 20061018
*/
class com.ionewmedia.utils.stage.Alineador_CentroAbajo {
	//--------------------
	// DOCUMENTACION:
	// Pasado una instancia de MovieClip como parametro, la clase se encarga de alinearlo abajo al centro
	...
	//--------------------
	// CONSTRUCTORA:
	function Alineador_CentroAbajo(dmc:MovieClip, an:Number, al:Number, sec:Number, ease:String) {
		// (donde_mc, ancho, alto, segundos, tipoAnim, aling)
		trace("(Alineador_CentroAbajo.CONSTRUCTORA): " + dmc._name);
		donde_mc = dmc;
		ancho = an;
		alto = al;
		segundos = sec;
	...
	private function ejecutar() {
		//trace("(Alineador_CentroAbajo.ejecutar): "+donde_mc._name);
		if (segundos == 0) {
			ejecutarSinAnim();
		} else {
			ejecutarConAnim();
	...
	public function get_arrayPos():Array {
		...
		var nuevaPosX:Number = Math.round((anchoStage / 2) - (ancho / 2));
