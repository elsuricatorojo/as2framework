...
* Creada: 20061018
*/
class com.ionewmedia.utils.stage.Alineador_DerArriba {
	//--------------------
	// DOCUMENTACION:
	...
	//--------------------
	// CONSTRUCTORA:
	function Alineador_DerArriba(dmc:MovieClip, an:Number, al:Number, sec:Number, ease:String) {
		// (donde_mc, ancho, alto, segundos, tipoAnim, aling)
		trace("(Alineador_DerArriba.CONSTRUCTORA): " + dmc._name);
		donde_mc = dmc;
		ancho = an;
		alto = al;
		segundos = sec;
	...
	private function ejecutar() {
		//trace("(Alineador_DerArriba.ejecutar): "+donde_mc._name);
		if (segundos == 0) {
			ejecutarSinAnim();
		} else {
			ejecutarConAnim();
