...
* Creada: 20061018
*/
class com.ionewmedia.utils.stage.Alineador_IzqAbajo {
	//--------------------
	// DOCUMENTACION:
	...
	//--------------------
	// CONSTRUCTORA:
	function Alineador_IzqAbajo(dmc:MovieClip, an:Number, al:Number, sec:Number, ease:String) {
		// (donde_mc, ancho, alto, segundos, tipoAnim, aling)
		trace("(Alineador_IzqAbajo.CONSTRUCTORA): " + dmc._name);
		donde_mc = dmc;
		ancho = an;
		alto = al;
		segundos = sec;
	...
	private function ejecutar() {
		//trace("(Alineador_IzqAbajo.ejecutar): "+donde_mc._name);
		if (segundos == 0) {
			ejecutarSinAnim();
		} else {
			ejecutarConAnim();
