...
* Creada: 20061018
*/
class com.ionewmedia.utils.stage.Alineador_IzqArriba {
	//--------------------
	// DOCUMENTACION:
	// Pasado una instancia de MovieClip como parametro, la clase se encarga de alinearlo arriba a la izquierda
	...
	//--------------------
	// CONSTRUCTORA:
	function Alineador_IzqArriba(dmc:MovieClip, an:Number, al:Number, sec:Number, ease:String) {
		// (donde_mc, ancho, alto, segundos, tipoAnim, aling)
		trace("(Alineador_IzqArriba.CONSTRUCTORA): " + dmc._name);
		donde_mc = dmc;
		ancho = an;
		alto = al;
		segundos = sec;
	...
	private function ejecutar() {
		//trace("(Alineador_IzqArriba.ejecutar): "+donde_mc._name);
		if (segundos == 0) {
			ejecutarSinAnim();
		} else {
			ejecutarConAnim();
	...
	public function get_arrayPos():Array {
		...
		//var nuevaPosX:Number = Math.round((anchoStage) + (ancho));
		var nuevaPosX:Number = Math.round(0 + (ancho/2));
		var nuevaPosY = Math.round(0 +(alto/2));
		//-----
